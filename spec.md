...
- zobaczenie panelu diagnostyki bezpośrednio pod headerem oraz jasnego aktywnego stanu przycisku `Diagnostyka`, gdy panel jest otwarty
...
Po przyroście `prd/008-panel-diagnostyki-pod-headerem.md` diagnostyka pozostaje tą samą funkcją i tym samym zestawem danych, ale panel jest traktowany jako globalny panel pod headerem. Otwarcie panelu nie zmienia aktualnego widoku albumów albo szczegółu albumu, tylko pokazuje diagnostykę nad bieżącą treścią i spycha ją niżej.
...
- położenie panelu diagnostyki pod headerem, aktywny stan przycisku `Diagnostyka`, poprawne `aria-expanded`, brak utrwalania stanu otwarcia i brak regresji `Test polaczenia`
...
- Decyzja: Panel diagnostyki będzie globalnym panelem statycznego frontendu umieszczonym pod headerem, a przycisk `Diagnostyka` będzie jawnie odzwierciedlał stan otwarcia. (dotyczy PRD: 008-panel-diagnostyki-pod-headerem.md)
- Uzasadnienie: PRD 008 rozwiązuje problem trudnego odnalezienia diagnostyki po kliknięciu przycisku, gdy panel znajduje się nisko pod albumami, oraz wymaga czytelnego stanu aktywnego bez zmian backendu i API.
- Konsekwencje: Implementacja powinna zachować istniejącą zawartość diagnostyki, `Test polaczenia` jako osobną akcję w headerze i brak nowych zależności. Stan otwarcia nie jest utrwalany, fokus po otwarciu zostaje na przycisku, `aria-expanded` odpowiada widoczności panelu, a animacja musi respektować `prefers-reduced-motion`.
- Dotyczy PRD / milestone’u: `prd/008-panel-diagnostyki-pod-headerem.md`; Milestone 17.
...
- panel diagnostyki musi być łatwo dostępny pod headerem, nie pod siatką albumów, a przycisk `Diagnostyka` musi czytelnie komunikować stan otwarcia
